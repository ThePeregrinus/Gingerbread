...
export function Table() {
  ...
  const columns: GridColDef<(typeof rows)[number]>[] = [
    {
      ...
      width: 70,
      sortable: false,
      renderCell: (row) => <DeleteButton id={row.row.id} />,
    },
    {
      field: "setting",
      headerName: "Setting",
      sortable: false,
      renderCell: (row) => <EditButton id={row.row.id} row={row.row} />,
    },
  ...
  const rows = data.map((column, icolumn) => {
    return { ...column, numeric: icolumn + 1 };
  });
